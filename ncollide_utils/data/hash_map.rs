...
impl<K, V, H: HashFun<K>> HashMap<K, V, H> {
    ...
    pub fn new_with_capacity(capacity: usize, h: H) -> HashMap<K, V, H> {
        ...
        HashMap {
            ...
            mask:   pow2 - 1,
            htable: iter::repeat(-1).take(pow2).collect(),
            next:   iter::repeat(-1).take(pow2).collect(),
            num_elem: 0,
            max_elem: pow2,
...
impl<K: PartialEq, V, H: HashFun<K>> HashMap<K, V, H> {
    ...
    fn may_grow(&mut self) {
        if self.num_elem >= self.real_max_elem {
            ...
            let mut newhash: Vec<isize> = iter::repeat(-1).take(self.max_elem).collect();
            let mut newnext: Vec<isize> = iter::repeat(-1).take(self.max_elem).collect();
